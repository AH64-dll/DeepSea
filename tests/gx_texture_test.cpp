...
int main()
{
  ...
  // Bounds guards must fail closed: a truncated encoded span, an undersized
  // palette, zero dimensions, and a null output all refuse to decode.
  const std::span<const std::uint8_t> truncated_i4(i4.data(), i4.size() - 1u);
  if (moderngekko::GxTextureDecoder::Decode(truncated_i4, 8, 8,
                                             moderngekko::GxTextureFormat::I4, {},
                                             moderngekko::GxPaletteFormat::IA8, &decoded))
  {
    return 14;
  }
  const std::span<const std::uint8_t> short_palette(palette.data(), 30u);
  if (moderngekko::GxTextureDecoder::Decode(c4, 8, 8,
                                             moderngekko::GxTextureFormat::C4,
                                             short_palette,
                                             moderngekko::GxPaletteFormat::IA8, &decoded))
  {
    return 15;
  ...
  if (moderngekko::GxTextureDecoder::Decode(i4, 0, 8,
                                             moderngekko::GxTextureFormat::I4, {},
                                             moderngekko::GxPaletteFormat::IA8, &decoded) ||
      moderngekko::GxTextureDecoder::Decode(i4, 8, 8,
                                             moderngekko::GxTextureFormat::I4, {},
                                             moderngekko::GxPaletteFormat::IA8, nullptr))
  {
    return 17;
  }
  // 2^20 x 2^20 overflows size_t on 32-bit hosts; EncodedSize must report 0
  // and Decode must refuse rather than wrap the size check.
  if (moderngekko::GxTextureDecoder::EncodedSize(1u << 20, 1u << 20,
                                                  moderngekko::GxTextureFormat::RGBA8) != 0u &&

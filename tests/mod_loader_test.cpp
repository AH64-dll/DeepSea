...
int main() {
  ...
  if (manager.Dispatch(&state, 0x80003000u))
    return 7;
  // R-0: entry hooks persist (frame60 accumulator needs pc=lr gates).
  // EntryHook writes gpr[3]=0xDEADBEEF and must survive Dispatch.
  if (entry_hooks != 1 || state.gpr[3] != 0xDEADBEEFu)
    return 8;
  if (!manager.HandlesAddress(state.lr))
    return 18;
  if (manager.Dispatch(&state, state.lr))
    return 9;
  // Return hooks remain observer-only; entry-hook value persists.
  if (return_hooks != 1 || state.gpr[3] != 0xDEADBEEFu)
    return 10;
  if (manager.HandlesAddress(state.lr))
    return 19;
  ...
  if (!manager.TriggerEvent("base_mod", "tick", &state))
    return 11;
  // Event callbacks remain observer-only; entry-hook value still persists.
  if (event_callbacks != 1 || state.gpr[3] != 0xDEADBEEFu)
    return 12;

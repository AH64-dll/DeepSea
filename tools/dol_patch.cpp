...
// Error text is shown in UTF-8 UI surfaces; fs::path::string() would emit the
...
bool LoadManifest(const fs::path &path, std::vector<Patch> *patches,
  ...
  if (!input) {
    *error = "can't open DOL patch manifest " + PathUtf8(path);
    return false;
  }
  std::string line;
  ...
  if (!line.empty() && line.back() == '\r')
    line.pop_back();
  // Tolerate a UTF-8 BOM from an editor-saved manifest (Notepad writes one).
  std::string_view header(line);
  if (header.starts_with("\xEF\xBB\xBF"))
...
bool ReadFile(const fs::path &path, std::vector<std::uint8_t> *bytes,
  ...
  if (!input) {
    *error = "can't open " + PathUtf8(path);
    return false;
  }
  ...
  try {
    ...
    *error = "can't read " + PathUtf8(path) + " (file too large)";
    return false;
  }
  input.seekg(0);
...
bool ReplaceFile(const fs::path &path, const std::vector<std::uint8_t> &bytes,
  ...
  fs::remove(temporary, ec);
  {
    ...
    if (!output || !output.write(reinterpret_cast<const char *>(bytes.data()),
      ...
      // Close before remove: Windows refuses to unlink an open file, which
      // would leave a partial .patching file next to the DOL.
      output.close();
      fs::remove(temporary, ec);
      *error = "can't write " + PathUtf8(temporary);
      return false;
    }
  }
  ...
  if (!MoveFileExW(temporary.c_str(), path.c_str(),
    ...
    fs::remove(temporary, ec);
    *error = "can't publish patched " + PathUtf8(path);
    return false;
  }
#else
...
bool ApplyDolPatchManifest(const fs::path &dol_path,
  ...
  *changed = false;
  ...
  for (const Patch &patch : patches) {
    ...
    // u64 math: mapped->offset + patch.address could wrap a u32 for a
    // pathological multi-GB input file before the subtraction.
    const std::size_t offset = static_cast<std::size_t>(
        static_cast<std::uint64_t>(mapped->offset) + patch.address -
        mapped->address);

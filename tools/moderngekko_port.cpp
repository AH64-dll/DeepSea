...
{
  if (const auto xdg = moderngekko::GetEnvPath("XDG_CACHE_HOME"))
    return *xdg / "moderngekko" / "modules";
  if (const auto home = moderngekko::GetEnvPath("HOME"))
    return *home / ".cache" / "moderngekko" / "modules";
...
// Quotes one argument for the command line built by RunCommand. `text` is
// UTF-8; RunCommand converts the finished line to UTF-16 for CreateProcessW,
// so no ANSI loss happens anywhere in the chain.
std::string Quote(std::string_view text)
{
#if defined(_WIN32)
  // CommandLineToArgvW rules: a run of N backslashes immediately before a
  // quote emits N/2 literal backslashes and treats '\"' as an escaped quote,
  // so a trailing "C:\dir\" (legal user input, e.g. argv[2]) would corrupt
  // the child's whole command line. Double every backslash run that precedes
  // a quote and escape embedded quotes.
  std::string result = "\"";
...
// Named separately from Quote(std::string_view): a single Quote() taking
// either type would make Quote(std::string) ambiguous (two user-defined
// conversion sequences).
std::string QuotePath(const fs::path& value)
{
  // UTF-8, not .string(): the Windows branch would emit the ANSI code page,
  // which cannot represent every user path.
  return Quote(moderngekko::PathToUtf8(value));
}
...
DolPatchSet LoadDefaultDolPatches(const fs::path& path)
{
  ...
  for (const std::string& current : lines)
  {
    ...
    // A patch line before any $Name has no identity: patch_name is "" there,
    // and a bare "$" line inside [OnFrame_Enabled] would otherwise enable it.
    if (current.empty() || current.starts_with('#') || current.starts_with(';') ||
        patch_name.empty() || !enabled.contains(patch_name))
      continue;
    ...
    {
      patches.error =
          "unsupported enabled DOL patch line in " + moderngekko::PathToUtf8(path);
      return patches;
    }
    DolPatch patch{};
    ...
    {
      patches.error =
          "malformed enabled DOL patch line in " + moderngekko::PathToUtf8(path);
      return patches;
    }
              ...
              const DolPatchSet& patches, std::string* error)
{
  ...
  if (!input)
  {
    *error = "can't open " + moderngekko::PathToUtf8(input_path);
    return false;
  }
  const std::streamoff input_size = input.tellg();
  if (input_size < 0x100)
  {
    *error = "malformed DOL " + moderngekko::PathToUtf8(input_path);
    return false;
  }
  ...
  {
    *error = "can't read " + moderngekko::PathToUtf8(input_path) +
             " (file too large)";
    return false;
  }
  input.seekg(0);
  ...
  {
    *error = "can't write " + moderngekko::PathToUtf8(output_path);
    return false;
  }
  return true;
}
...
bool RunCommand(const std::string& command)
{
  std::cout << "+ " << command << '\n';
#if defined(_WIN32)
  // `command` is UTF-8 (Quote emits UTF-8). CreateProcessA would decode the
  // command line as ANSI and mangle non-ANSI paths, so go through the wide
  // API; CommandLineToArgvW applies the same quoting rules either way.
  const std::wstring wide_command = moderngekko::Utf8ToWide(command);
  if (wide_command.empty())
  {
    std::cerr << "can't convert command line to UTF-16\n";
    return false;
  }
  std::vector<wchar_t> command_line(wide_command.begin(), wide_command.end());
...
{
  std::error_code ec;
  // argv0 is UTF-8 like every narrow string in this tool.
  fs::path self = fs::weakly_canonical(moderngekko::Utf8ToPath(argv0), ec);
#if defined(_WIN32)
  name += ".exe";
#endif
                               ...
                               const moderngekko::GameMetadata& game)
{
  ...
  // active-module.txt is written UTF-8 below; decode it the same way.
  const fs::path module = moderngekko::Utf8ToPath(active);
  if (!fs::is_regular_file(module))
    return "missing: " + moderngekko::PathToUtf8(module);
  ...
  while (std::getline(manifest, line))
  {
    ...
    if (line.starts_with(prefix))
    {
      ...
      return std::string(current ? "current: " : "stale: ") +
             moderngekko::PathToUtf8(module);
    }
  }
  return "unverified: " + moderngekko::PathToUtf8(module);
}
                              ...
                              BuildOptions options)
{
  ...
  if (fs::is_regular_file(module))
  {
    ...
    active << moderngekko::PathToUtf8(module) << '\n';
  ...
  std::string generate = QuotePath(dolrecomp) + " -j" +
                         std::to_string(std::max(1u, std::thread::hardware_concurrency())) +
                         " --backend=" + options.backend + " ";
  if (game.platform == moderngekko::GamePlatform::GameCube)
    generate += "--cpu gekko --gamecube " + QuotePath(recomp_dol) + " " + QuotePath(generated_parent);
  else
    generate += "--cpu broadway " + QuotePath(recomp_dol) + " " + game.disc_id + " " +
                QuotePath(generated_parent);
  ...
  std::string configure = "cmake -E env CMAKE_NINJA_FORCE_RESPONSE_FILE=1 cmake -S " +
      QuotePath(source_root / "vendor/dolphin/module-template") +
      " -B " + QuotePath(module_build) + " -G Ninja -DCMAKE_BUILD_TYPE=Release" +
      " -DCMAKE_C_COMPILER=" + compiler + " -DGAME_ID=" + game.disc_id +
      " -DGENERATED_DIR=" + QuotePath(generated) +
      " -DGXRUNTIME_DIR=" + QuotePath(source_root / "vendor/dolphin/GXRuntime") +
      (options.opt_level.empty()
           ? std::string()
           : " -DRECOMPCORE_MODULE_OPT_LEVEL=" + options.opt_level) +
      " -DCHASSIS_ABI_DIR=" +
      QuotePath(source_root / "vendor/dolphin/Source/Core/Core/PowerPC/StaticRecomp");
  if (!RunCommand(configure) ||
      !RunCommand("cmake --build " + QuotePath(module_build) + " -j" +
...
int PortMain(int argc, char** argv)
{
#if defined(_WIN32)
  // The CRT encodes argv in the ANSI code page, which cannot represent paths
  // outside it (a game root under e.g. C:\Users\宮本). Rebuild argv from the
  // wide command line as UTF-8; every path decode in this tool goes through
  // Utf8ToPath. On failure keep the CRT argv rather than lose the tool.
  ...
  const fs::path root = moderngekko::Utf8ToPath(argv[2]);
  BuildOptions options;
  ...
  for (int i = 3; i < argc; ++i)
  {
    ...
    if (runner_args)
      ...
      options.output = moderngekko::Utf8ToPath(argv[++i]);
    else if (command == "run")
      options.runner_arguments.push_back(arg);
    else
    {
  ...
  if (command == "build")
    return 0;
  std::string run = QuotePath(SiblingExecutable(argv[0], "moderngekko-run")) + " --game " +
                    QuotePath(root) + " --module " + QuotePath(*module);

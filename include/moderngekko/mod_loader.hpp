...
public:
  ...
  static bool HostCallRangeContains(std::uint32_t start, std::uint32_t end,
                                    void *user_data);
  // Monotonic counter of verdict-mutating events (pending-return push/pop,
  // first-dispatch runtime_start). The chassis diffs it across a host_call
  // to skip the dispatch-epoch bump when nothing mutated.
  static std::uint64_t VerdictOps(void *user_data);
  // Drain of pcs whose dispatch verdict may have flipped since the last
  // drain. Returns the count written into out[0..cap), or ~0u on overflow /
  // a non-pc event — the chassis falls back to a global epoch bump then.
  static std::uint32_t VerdictPcs(void *user_data, std::uint32_t *out,
                                  std::uint32_t cap);

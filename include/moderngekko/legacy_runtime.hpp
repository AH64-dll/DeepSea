...
public:
  ...
  // Opens the default host audio output and wires it to the emulated DSP DMA
  // stream. Run() calls this lazily; a missing device degrades gracefully to
  // dropping samples. Returns false when no output is available.
  bool StartAudioBackend();
...
private:
  ...
  bool m_is_wii = false;
  std::unique_ptr<AudioOutput> m_audio_output;
};
}

...
// Scheduler ticks run at 323.712 MHz (AiCyclesPerSample=10116 <-> 32 kHz AI
// counter), so the 30 Hz guest video-frame boundary is an exact multiple of
// the tick grid; t=0 at boot is a boundary by construction.
constexpr std::uint64_t GuestFrameCycles = 323712000u / 30u;  // 10,790,400

// AU-1 Option A (phase3-au1-cadence-fix-recipe.md): default OFF. Read on
// every use (not cached): this runs only on AI MMIO writes / interrupt
// rescheduling, and an uncached read keeps the knob testable in-process.
bool AiFrameAlignEnabled()
{
  const char* env = std::getenv("MODERNGEKKO_AI_FRAME_ALIGN");
...
{
  // Scheduled lambdas capture `this`; a standalone AudioSystem can be
  // destroyed while its scheduler outlives it (same shape as the
  // DiscInterface fix), so cancel any pending events here.
  if (m_ai_event != 0)
...
void AudioSystem::ScheduleAiInterrupt()
{
  ...
  std::uint64_t delay = samples == 0u ? AiCyclesPerSample : samples * AiCyclesPerSample;
  if (AiFrameAlignEnabled())
  {
    // Snap the AI interrupt due time to the nearest guest video-frame
    // boundary so DSP frame completions quantize onto the 30 Hz grid the
    // same way retail does (retail derives AI timing from the frame clock).
    // Placement-only shift of < half a frame; steady-body sample content is
    // untouched because DMA delivery is content-identical regardless of when
    // the interrupt lands.
    const std::uint64_t now = m_scheduler.GetTicks();
    const std::uint64_t snapped =
        ((now + delay + GuestFrameCycles / 2) / GuestFrameCycles) * GuestFrameCycles;
    if (snapped > now)
      delay = snapped - now;
  }

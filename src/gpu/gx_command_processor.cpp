...
// Hard bound on the gather buffer. The retained tail of WriteBytes can only
// ever be an incomplete command (complete commands are always consumed);
// the largest valid command is a draw at 3 + 0xFFFF*vertex_size, and
// GetVertexSize sums to well under 256 bytes, so 32 MiB is generous
// headroom. A tail larger than that can never complete into a valid
// command — it is garbage that would otherwise grow the buffer without
// limit on a malformed stream.
constexpr std::size_t MaxBufferedBytes = 32u << 20;
...
void GxCommandProcessor::WriteBytes(std::span<const std::uint8_t> bytes)
{
  ...
  // Fail-closed: a buffered tail that can never complete (exceeds the
  // largest valid command) is dropped rather than grown without bound.

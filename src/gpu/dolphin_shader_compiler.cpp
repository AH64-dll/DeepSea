                    ...
                    const DolphinShaderOptions& options)
{
  ...
  // iEFBScale is int in VideoConfig: an unchecked u32->int cast wraps large
  // caller-supplied scales negative, and 0/negative scales are meaningless
  // anyway. Clamp to the sane IR range (retail UIs top out at 8x).
  g_ActiveConfig.iEFBScale =
      static_cast<int>(std::clamp(options.efb_scale, 1u, 16u));
}
}

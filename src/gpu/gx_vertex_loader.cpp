...
public:
  ...
  const std::uint8_t* Take(std::size_t size)
  {
    if (size > m_data.size() - m_offset)
      return nullptr;
    const std::uint8_t* const base = m_data.data();
    if (base == nullptr)
      return nullptr;  // size must be 0 here; avoid nullptr + 0 arithmetic
    const std::uint8_t* result = base + m_offset;
    m_offset += size;
    return result;
  }
...
{
  // packet.vat is a free u8 on the public struct; the VAT tables are
  // cp[0x70..0x9F], so anything above 7 would read out of bounds.
  if (output == nullptr || cp.size() < 256u || packet.vat > 7u)
    return false;
  // Vector growth below can throw bad_alloc; a bool-returning decoder
  // should fail instead of propagating through callers that never expect
  // an exception from Decode.
  try
  {
    return DecodeImpl(packet, cp, output);
  }
...
bool GxVertexLoader::DecodeImpl(const GxDrawPacket& packet,
  ...
  for (std::uint16_t vertex_index = 0; vertex_index < packet.vertex_count; ++vertex_index)
  {
    ...
    if ((normal_desc == 2u || normal_desc == 3u) && has_ntb && normal_index3)
    {
      ...
      for (std::uint32_t group = 0; group < 3u; ++group)
      {
        ...
        // A vertex discarded by a -1 position index still has its stream
        // bytes consumed (the index reads above) but must not fail the draw
        // on an array resolve — Dolphin's compiled loaders jump straight to
        // the next vertex stride without touching the remaining arrays.
        if (source == nullptr)
        {
          if (!skip_vertex)
            return false;
    ...
    else
    {
      ...
      if (normal_desc != 0u && normal == nullptr && !skip_vertex)
        return false;
      if (normal != nullptr)
      {
    ...
    for (unsigned color = 0; color < 2; ++color)
    {
      ...
      if (descriptor != 0u && source == nullptr && !skip_vertex) return false;

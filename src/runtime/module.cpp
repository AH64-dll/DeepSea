...
static const ModernGekkoRange* range_containing(const ModernGekkoRange* ranges,
...
static bool rel_modules_are_valid(const ModernGekkoModuleDesc* descriptor)
{
    ...
    for (module_index = 0; module_index < descriptor->num_rel_modules; ++module_index)
    {
        ...
        for (section_index = 0; section_index < module->num_sections; ++section_index)
        {
            ...
            const ModernGekkoRange* covering;
            uint64_t end = (uint64_t)section->linked_start + section->size;
            /* Checking linked_start and end-1 separately still lets a
             * section straddle the gap between two code ranges (start in
             * range A, last byte in range B, middle unmapped). Require one
             * range to contain the whole section, like chunks_tile_code
             * demands chunk.end <= code.end. */
            covering = range_containing(descriptor->code_ranges,
            ...
            if (section->module_id != module->module_id ||
                ...
                end > 0x100000000ull || covering == NULL || end > covering->end)
            {
                return false;
            }
        }

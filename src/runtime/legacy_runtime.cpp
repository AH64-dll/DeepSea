...
LegacyRuntime::~LegacyRuntime() = default;
...
LegacyRunResult LegacyRuntime::Run(std::uint64_t dispatch_limit)
{
  ...
  // Audio backend is enabled by default on the first run; a missing host
  // device degrades gracefully to the pre-wiring behavior (samples drop).
...
bool LegacyRuntime::StartAudioBackend()
{
  if (m_audio_output == nullptr)
    m_audio_output = CreateDefaultAudioOutput();
  return m_audio_output->Start(m_audio_system);
}
